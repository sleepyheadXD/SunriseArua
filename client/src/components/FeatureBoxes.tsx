  ...
  return (
    <div className="w-full flex flex-col gap-10 md:flex-row md:gap-6 max-w-4xl mx-auto justify-center items-center mt-14 mb-10 px-4">
      {features.map((feature, index) => (
        <motion.div
          key={feature.name}
          className={`relative w-full md:w-1/3 min-h-[220px] rounded-xl overflow-hidden cursor-pointer 
                     backdrop-blur-sm border border-white/20 bg-black/30 group`}
        ...
        >
          {/* Gradient background */}
          <div 
            className={`absolute inset-0 bg-gradient-to-br ${feature.color} opacity-0 group-hover:opacity-30 
          ...
          <div className="relative z-10 h-full flex flex-col items-center justify-center p-6 text-center">
            ...
            <div className="rounded-full p-4 bg-white/10 mb-4 relative">
              <feature.icon 
                className={`w-10 h-10 ${feature.iconColor} group-hover:text-white transition-colors duration-300`} 
              />
